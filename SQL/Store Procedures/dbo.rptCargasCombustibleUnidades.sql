...
BEGIN
    ...
    SELECT CASE
               WHEN ISNUMERIC(cc.UNIDAD) = 1 THEN
                   RIGHT('00000' + LTRIM(RTRIM(CONVERT(VARCHAR(10), CONVERT(INT, cc.UNIDAD)))), 5)
    ...
    FROM dbo.ConsumosCombustible AS cc
        INNER JOIN dbo.CatUnidadesReporte AS cu
            ON cu.sUnidad = (CASE
                                 WHEN ISNUMERIC(cc.UNIDAD) = 1 THEN
                                     CONVERT(VARCHAR(10), CONVERT(INT, cc.UNIDAD))
                                 WHEN ISNUMERIC(cc.UNIDAD) = 0 THEN
    ...
    /*SELECT  cc.UNIDAD AS 'Unidad' ,
						...
						cc.ORIGEN AS 'Origen'
                FROM    dbo.ConsumosCombustible AS cc
                        INNER JOIN dbo.CatUnidadesReporte AS cu ON cu.sUnidad = cc.UNIDAD AND cu.bActivo = 1
